...
import { Landmark, ShoppingCart, ArrowLeftRight, Settings, Users, LogOut, BellRing } from 'lucide-react';
    ...
    >
      ...
      <div className="mt-4 flex justify-between items-start">
        <div>
          ...
          <p className="text-xs text-gray-500">
            {currentUser?.role === 'owner' ? '👑 Account Owner' : '👥 Family Member'}
          </p>
        </div>
        <button
          onClick={logout}
      ...
      <div className="my-auto space-y-6 max-w-[360px] mx-auto w-full">
        ...
        <div className="space-y-4">
          ...
          {/* Give / Take — owner only */}
          {isOwner && (
            <button
              id="option_givetake_btn"
              onClick={() => navigate('give-take')}
              className="w-full flex items-center p-4 border border-gray-100 rounded-2xl bg-white hover:bg-gray-50 hover:border-teal-300 shadow-sm transition-all text-left group active:scale-[0.99]"
            >
              <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-teal-50 to-red-50 flex items-center justify-center mr-4 group-hover:from-teal-100 group-hover:to-red-100 transition-colors border border-teal-100">
                <ArrowLeftRight className="w-6 h-6 text-teal-600" />
              </div>
              <div>
                <h4 className="font-bold text-gray-900 text-sm">Give / Take</h4>
                <p className="text-xs text-gray-500 mt-0.5">Track money loaned out and borrowed</p>
              </div>
            </button>
          )}
        </div>
      ...
      {isOwner ? (
        ...
        currentUser?.permissions?.addBudget && (
          <div className="text-center">
            ...
            >
              <Settings className="w-3.5 h-3.5" />Member Toolbar
            </button>
          </div>
        )
      )}
